...
export const getDashboard = TryCatch(async (req,res,next)=>{
    ...
    if(myCache.has(key)) stats=JSON.parse(myCache.get(key)as string);
        ...
        const lastestTransactionPromise =  Order.find({}).select
        ...
        const changepercent ={
            ...
            user:calculatePercentage(thisMonthUsers.length, 
                lastMonthUsers.length,
            ),

            order:calculatePercentage(thisMonthOrders.length, 
...
export const getPieCharts = TryCatch(async (req,res,next)=>{
    ...
    if(myCache.has(key)) 
        ...
        (["total","discount", "subtotal", "tax","shippingCharges"])
